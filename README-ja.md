...
**XP3**（吉里吉里）、**PFS**（Artemis）、**NSA/SAR**（NScripter）、**YPF**（YU-RIS）、**ZIP**、**7z**、**ISO 9660** をサポート。Rust ネイティブコア。
...
| 📜 **NSA/SAR** | NScripter `.nsa` / `.sar` を解凍（zlib圧縮対応） |
| 📦 **YPF** | YU-RIS `.ypf` を3層検出で解凍 |
| 🗜️ **ZIP/7z 圧縮** | ZIP/7z 圧縮、5段階レベル、AES-256暗号化 |
| 🔐 **解凍パスワード** | 暗号化ZIP/7zのパスワード入力対応 |
...
本ツールは合法的に所有するファイルの個人使用のみを目的としています。
